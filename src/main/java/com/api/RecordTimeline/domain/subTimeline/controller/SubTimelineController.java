...
@RestController
@RequestMapping("/api/sub-timelines")
public class SubTimelineController {
    @Autowired
    private SubTimelineService subTimelineService;

    @PostMapping
    public ResponseEntity<SubTimeline> createSubTimeline(@RequestBody SubTimeline subTimeline) {
        SubTimeline created = subTimelineService.createSubTimeline(subTimeline);
